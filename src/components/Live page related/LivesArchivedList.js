...
import {LiveArchiveCard} from "../index";
import {LinearProgress} from "@mui/material";


const LivesArchivedList = ({cityid, singerid}) => {
    ...
    const getdata = async (page) => {
        ...
        const options = {
            ...
            params: {page: page, take: 4, live: 0, city_id: cityid, search: search, singer_id: singerid}
        };
        setLoading(true)
    ...
    useEffect(() => {
        ...
        const getdatabegin = async (city) => {
            const options = {
                params: {take: 4, live: 0, city_id: city, search: search}
            };
            setLoading(true)
            const response = await axios.get(endpoint, options)
            if (response.data.code === 1) {
...
// fires when search statae is updated
    ...
    }, [cityid, search, singerid]);


    let content
    if (loading === false) {
        content = data.items.map((item) => (
            <LiveArchiveCard key={item.videos_view.id} singer={item.singers.name} title={item.videos_view.title}
                             id={item.videos_view.id} price={item.videos_view.price}
                             location={item.videos_view.location_name} date={item.videos_view.date_text}
                             img={item.videos_view.cover} city={item.cities.name}/>
        ))
    } else {
...
export default LivesArchivedList;